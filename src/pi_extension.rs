...
use crate::paths::{home_dir, pi_extension_path, pi_extension_path_from_home};
...
pub fn install_global_pi_extension() -> Result<PiExtensionInstallResult, Box<dyn Error>> {
    install_global_pi_extension_at(&pi_extension_path()?)
}

pub fn restore_global_pi_extension() -> Result<PiExtensionRestoreResult, Box<dyn Error>> {
    restore_global_pi_extension_at(&pi_extension_path()?)
}
...
-> Result<PiExtensionSyncStatus, Box<dyn Error>> {
    ...
    synchronize_global_pi_extension_if_installed_at(&pi_extension_path_from_home(&home)?)
}

fn install_global_pi_extension_at(path: &Path) -> Result<PiExtensionInstallResult, Box<dyn Error>> {
...
fn restore_global_pi_extension_at(path: &Path) -> Result<PiExtensionRestoreResult, Box<dyn Error>> {
    if !path.exists() {
...
mod tests {
    use super::*;
    use crate::paths::pi_extension_path_from_home;
    ...
    fn installs_bundled_pi_extension_into_global_path() {
        ...
        let result = install_global_pi_extension_at(&path).unwrap();
    ...
    fn restore_removes_installed_global_pi_extension() {
        ...
        let result = restore_global_pi_extension_at(&path).unwrap();
    ...
    fn sync_updates_existing_outdated_global_pi_extension() {
        ...
        let result = synchronize_global_pi_extension_if_installed_at(&path).unwrap();
