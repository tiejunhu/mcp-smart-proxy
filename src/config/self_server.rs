...
pub(crate) fn inspect_crush_self_server(
    servers: &JsonMap<String, JsonValue>,
    provider: &str,
) -> Option<(String, bool)> {
    inspect_self_server(
        servers.iter().filter_map(|(name, value)| {
            let server = value.as_object()?;
            let raw_command = crush_server_raw_command(server)?;
            Some((name.clone(), raw_command))
        }),
        provider,
    )
...
pub(crate) fn inspect_omp_self_server(
    servers: &JsonMap<String, JsonValue>,
    provider: &str,
) -> Option<(String, bool)> {
    inspect_self_server(
        servers.iter().filter_map(|(name, value)| {
            let server = value.as_object()?;
            let raw_command = omp_server_raw_command(server)?;
            Some((name.clone(), raw_command))
        }),
        provider,
    )
...
pub(crate) fn crush_server_raw_command(server: &JsonMap<String, JsonValue>) -> Option<Vec<String>> {
    match server
        .get("type")
        .and_then(JsonValue::as_str)
        .unwrap_or("stdio")
...
pub(crate) fn omp_server_raw_command(server: &JsonMap<String, JsonValue>) -> Option<Vec<String>> {
    match server
        .get("type")
        .and_then(JsonValue::as_str)
        .unwrap_or("stdio")
