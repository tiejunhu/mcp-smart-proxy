...
- Keep popup footer actions explicit in the Swift helper: even though the dialog may auto-advance or auto-finish after a valid selection, the footer should still expose a primary action so typed `Other` answers can be submitted without relying on keyboard Return alone, and that button label should reflect the workflow outcome such as continuing or finishing rather than a generic mechanism label.
