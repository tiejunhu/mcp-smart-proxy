...
- Keep popup progression sequential in the Swift helper: opening the dialog must not preselect any answer, only the current question should be interactive, each active question starts a 10-second countdown, and if that countdown expires without user interaction the first option is selected automatically before the flow advances.
- Keep popup keyboard selection deterministic in the Swift helper: assign dialog-wide `1-9a-z` shortcuts in display order until that shortcut set is exhausted, let plain shortcut keys act only on the current question whenever no custom input is focused, treat Return inside `Other` as confirm-and-advance, and complete the dialog automatically after the final answer is confirmed.
