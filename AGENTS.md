...
- Keep Crush support split by concern: `src/config/import_export/crush.rs` owns Crush `crush.json` parsing/install/restore details (top-level `mcp` key, `stdio`/`http`/`sse` transports, `disabled` flag, `CRUSH_GLOBAL_CONFIG` path override), while programmatic summary generation stays in `src/reload/summarizer.rs` via `crush run --quiet`, and Crush-only fields such as `disabled_tools` or `timeout` that `msp` cannot preserve must fail import explicitly instead of being silently dropped.
- Keep Oh My Pi (omp) support split by concern: `src/config/import_export/omp.rs` owns Oh My Pi `mcp.json` parsing/install/restore details (top-level `mcpServers` key, `stdio`/`http`/`sse` transports, per-server `enabled` flag, `PI_CODING_AGENT_DIR` directory override resolved to `$PI_CODING_AGENT_DIR/mcp.json`), while programmatic summary generation stays in `src/reload/summarizer.rs` via `omp launch --print`, and omp-only fields such as `cwd`, `timeout`, `auth`, or `oauth` that `msp` cannot preserve must fail import explicitly instead of being silently dropped.
...
- Keep `msp cli` human-friendly: render `structuredContent` as TOON only when `--output-toon` is requested, print a single plain text content item directly when no structured payload exists, and fall back to JSON only for mixed or non-text results.
